...
App.Views.Canvas = Backbone.View.extend({
  ...
  flatten: function(){
    ...
    this.patch.nodes.each(function(x){
      var j = x.toJSON();
      flat.push([j.id, Number(j.attrs.x), Number(j.attrs.y)]);
    });
  },
  events: {
  ...
  mousedown: function(e){
    ...
    if(this.hoverNode && this.connecting){
      ...
      // do connect here
      this.toNode = this.hoverNode;
      // this.connect(this.fromNode[0], this.toNode[0]);
  ...
  draw: function(){   
    ...
    ctx.fillText(this.hoverNode + ' ' + this.fromNode + ' ' + this.toNode, 20, 60);
    ctx.fillText(this.hoverNode + ' ' + this.fromNode + ' ' + this.toNode, 20, 60);
